...
const SYMBOL_PRIORITY_BY_EXCHANGE = {
...
export function resolveSymbolFromEnv(explicitExchangeId) {
...
const resolveBasisSymbol = (envKeys, fallback) => {
    for (const key of envKeys) {
        const value = process.env[key];
        if (value && value.trim()) {
...
const resolveGridDirection = (raw, fallback) => {
    if (!raw)
        return fallback;
    const normalized = raw.trim().toLowerCase();
    if (normalized === "long" || normalized === "long-only")
        return "long";
    if (normalized === "short" || normalized === "short-only")
        return "short";
    if (normalized === "both" || normalized === "dual" || normalized === "bi" || normalized === "two-way")
        return "both";
    return fallback;
};
const resolveGridMaxPosition = (orderSize, levels) => {
...
export const hedgeConfig = {
    grvtSymbol: (process.env.HEDGE_GRVT_SYMBOL && process.env.HEDGE_GRVT_SYMBOL.trim()) ||
        resolveSymbolFromEnv("grvt"),
    bingxSymbol: (process.env.HEDGE_BINGX_SYMBOL && process.env.HEDGE_BINGX_SYMBOL.trim()) ||
        resolveSymbolFromEnv("bingx"),
    orderAmount: Math.max(0, parseNumber(process.env.HEDGE_ORDER_AMOUNT ?? process.env.TRADE_AMOUNT, 0.001)),
    ...
    grvtPriceTick: Math.max(1e-9, parseNumber(process.env.HEDGE_GRVT_PRICE_TICK ?? process.env.PRICE_TICK, 0.1)),
    grvtQtyStep: Math.max(1e-9, parseNumber(process.env.HEDGE_GRVT_QTY_STEP ?? process.env.QTY_STEP, 0.001)),
    bingxPriceTick: Math.max(1e-9, parseNumber(process.env.HEDGE_BINGX_PRICE_TICK ?? process.env.PRICE_TICK, 0.1)),
    bingxQtyStep: Math.max(1e-9, parseNumber(process.env.HEDGE_BINGX_QTY_STEP ?? process.env.QTY_STEP, 0.001)),
};
export function isBasisStrategyEnabled() {
