export function toDepth(symbol, snapshot) {
    const toLevels = (levels) => levels.map((level) => [level.price, level.size]);
    return {
        symbol,
...
export function toTicker(symbol, stats) {
    return {
        symbol,
...
export function toKlines(symbol, interval, klines) {
...
export function toOrders(symbol, orders) {
    return orders.map((order) => lighterOrderToAster(symbol, order));
}
export function lighterOrderToAster(symbol, order) {
    const side = order.is_ask || order.side?.toLowerCase() === "sell" || order.side?.toLowerCase() === "ask"
        ? "SELL"
        : "BUY";
    return {
...
function computeExecutedQty(order) {
    if (order.filled_base_amount)
        return order.filled_base_amount;
...
export function toAccountSnapshot(symbol, details, positions = [], assets = [], options) {
    const targetSymbol = options?.marketSymbol ?? null;
    const targetMarketId = options?.marketId != null && Number.isFinite(Number(options.marketId))
...
function lighterPositionToAster(symbol, position) {
...
function symbolsMatch(source, target) {
    if (!source || !target)
        return false;
...
function normalizeSymbolForms(value) {
