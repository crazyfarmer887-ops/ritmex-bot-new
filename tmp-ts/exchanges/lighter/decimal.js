...
export function scaleQuantityWithMinimum(value, decimals) {
...
export function scaledToDecimalString(value, decimals) {
    if (typeof value === "number") {
        if (!Number.isFinite(value))
            throw new Error(`Invalid scaled number: ${value}`);
...
export function clampToInt64(value) {
    const max = (1n << 63n) - 1n;
    const min = -(1n << 63n);
...
export function safeNumberToUint32(value) {
