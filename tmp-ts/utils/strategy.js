export function getPosition(snapshot, symbol) {
    if (!snapshot) {
    ...
    const selected = withExposure.find((p) => p.positionSide === "BOTH") ??
...
export function getSMA(values, length) {
...
export function calcStopLossPrice(entryPrice, qty, side, loss) {
    if (side === "long") {
        return entryPrice - loss / qty;
    }
    return entryPrice + loss / Math.abs(qty);
}
export function calcTrailingActivationPrice(entryPrice, qty, side, profit) {
    if (side === "long") {
        return entryPrice + profit / qty;
    }
    return entryPrice - profit / Math.abs(qty);
}
export function computeBollingerBandwidth(values, length, stdMultiplier) {
...
export function isOrderPriceAllowedByMark(params) {
    ...
    if (!Number.isFinite(price) || !Number.isFinite(mark) || mark <= 0)
        return true;
    if (side === "BUY") {
