...
const READY_MESSAGE = t("正在等待交易所推送数据…");
...
export function TrendApp({ onExit }: TrendAppProps) {
  ...
  const orderColumns: TableColumn[] = [
    ...
    { key: "type", header: t("Type"), minWidth: 10 },
  ...
  return (
    <Box flexDirection="column" paddingX={1} paddingY={0}>
      <Box flexDirection="column" marginBottom={1}>
        <Text color="cyanBright">{t("Trend Strategy Dashboard")}</Text>
        <Text>
          {t("交易所:")} {exchangeName} ｜ {t("交易对:")} {snapshot.symbol} ｜ {t("最近价格:")} {formatNumber(lastPrice, 2)} ｜ {t("SMA30")} {formatNumber(sma30, 2)} ｜ {t("趋势:")} {trend}
        </Text>
        <Text color="gray">{t("状态:")} {ready ? t("实时运行") : READY_MESSAGE} ｜ {t("按 Esc 返回策略选择")}</Text>
      </Box>
      ...
      <Box flexDirection="row" marginBottom={1}>
        <Box flexDirection="column" marginRight={4}>
          ...
          {hasPosition ? (
            <>
              <Text>
                {t("方向:")} {position.positionAmt > 0 ? t("多") : t("空")} ｜ {t("数量:")} {formatNumber(Math.abs(position.positionAmt), 4)} ｜ {t("开仓价:")} {formatNumber(position.entryPrice, 2)}
              </Text>
              <Text>
                {t("浮动盈亏:")} {formatNumber(snapshot.pnl, 4)} USDT ｜ {t("账户未实现盈亏:")} {formatNumber(snapshot.unrealized, 4)} USDT
              </Text>
            </>
          ) : (
        ...
        <Box flexDirection="column">
          <Text color="greenBright">{t("绩效")}</Text>
          <Text>
            {t("累计交易次数:")} {snapshot.totalTrades} ｜ {t("累计收益:")} {formatNumber(snapshot.totalProfit, 4)} USDT
          </Text>
          <Text>
            {t("累计成交量:")} {formatNumber(sessionVolume, 2)} USDT
          </Text>
          {snapshot.lastOpenSignal.side ? (
            <Text color="gray">
              {t("最近开仓信号:")} {snapshot.lastOpenSignal.side} @ {formatNumber(snapshot.lastOpenSignal.price, 2)}
            </Text>
          ) : null}
        </Box>
      </Box>
      ...
      <Box flexDirection="column">
        <Text color="yellow">{t("最近交易与事件")}</Text>
        {lastLogs.length > 0 ? (
